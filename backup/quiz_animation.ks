...
// ゲージの見た目を初期化（幅は450px固定のまま、マスクを解除）
$(".time_gage").stop().css({
    "transition": "none",
    "width": "450px",            // 幅は常に固定
    "clip-path": "inset(0 0% 0 0)", // 全体表示（上 右 下 左 のカット率）
    "opacity": "1",
    "display": "block"
});
...
setTimeout(function(){
    $(".time_gage").css({
        ...
        "clip-path": "inset(0 100% 0 0)" // 右側から100%分マスクして消す
    });
}, 20);
[endscript]
...
; ★ ティラノ純正の待機タグで時間をカウント（これが一番安全です）
[wait time="&tf.wait_time"]

; 時間切れになった場合、そのまま下の *time_up に流れる
