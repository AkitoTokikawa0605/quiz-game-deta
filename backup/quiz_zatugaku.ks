...
if(tf.diff == "easy"){
    ...
    tf.current_bgm = 'zatsugaku_easy.ogg';
    ...
    tf.current_bgm = 'zatsugaku_normal.ogg';
    ...
    tf.current_bgm = 'zatsugaku_hard.ogg';
    ...
    tf.current_bgm = 'zatsugaku_very_hard.ogg';
}
...
[if exp="sf.selected_chara == 'hina'"]
    ...
    [stopse]
    [playse storage="&f.wel_v2"]
    [emb exp="tf.diff_name"]の問題は全20問から10問出題されるよ！[r]
    ※難易度が高くなるほど、今まで出題されていた問題も出ます。[p]
...
; キャラ立ち絵（通常顔）描画
[iscript]
var chara = sf.selected_chara || 'zunda';
tf.chara_normal = chara + "/" + chara + "_normal.png";
[endscript]

[freeimage layer="1"]
[layopt layer="1" visible="true"]
[image storage="&tf.chara_normal" layer="1" x="900" y="100" width="400" name="chara_stand"]
...
[freeimage layer="1"]
[layopt layer="1" visible="true"]
[image storage="&tf.chara_sad" layer="1" x="900" y="100" width="400" name="chara_stand"]
...
[iscript]
tf.current_genre = 'zatsugaku'; // ★zatsugakuに変更
...
if (tf.diff === 'easy' && !sf[key_normal]) {
    sf.zatsugaku_easy = true; // ★雑学EASYクリアフラグ
    sf[key_normal] = true;
