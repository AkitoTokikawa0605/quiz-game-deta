...
if (tf.life > 0) {
    ...
    if (tf.diff === 'easy') {
        ...
        if (!sf[key_normal] && !sf.trivia_normal) {
            sf[key_normal]   = true;
    ...
    if (tf.diff === 'normal' && tf.score >= 8 && !sf[key_hard] && !sf.trivia_hard) {
        sf[key_hard]    = true;
        sf.trivia_hard  = true;
        tf.show_dialog  = true;
    ...
    if (tf.diff === 'hard' && tf.score >= 8 && tf.life == tf.max_life && !sf[key_veryhard] && !sf.trivia_veryhard) {
