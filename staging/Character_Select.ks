...
[if exp="f.current_idx == 5"]
    [if exp="sf.unlock_zunda == true"]
        [image storage="zunda/zunda_normal.png" layer="2" x="380" y="150" width="500"]
    [else]
...
tf.dlg_msg = "このキャラクターはまだ解放されていません！\n全ジャンルのEASYをクリアしてください。";
[endscript]
...
[else]
    ...
    tf.confirm_voice = "voice/" + sf.selected_chara + "/confirm_voice.ogg";
    tf.restore_vol = (sf.bgm_vol !== undefined) ? sf.bgm_vol : 100;
    [endscript]
    ...
    [playse storage="&tf.confirm_voice"]
    [wait time="2000"]

    ; BGM音量を戻す
    [bgmopt volume="&tf.restore_vol"]
